...
6.nginx解决路由mode为history为404问题

```nginx
location / {
     root /home/ubuntu/myproject/view/video/dist;
     index index.html;
     //下边代码是解决404问题
...
```javascript
//使用vue-cli解决本地开发跨域的问题  配置文件在config/index文件
//配置前请求示例
axios.get('http://localhost:3000/test').then(res=>{console.log(res)})  //这样会提示跨域
//配置文件如下
proxyTable : {
  '/api' : {
        target : 'http://localhost:3000',//需要代理的域名和端口号，下次请求/api会代替当前的域名和端口
...
//配置后请求示例
axios.get('/api/test').then(res=>{console.log(res)})  //不会提示跨域
//解决完毕
```
